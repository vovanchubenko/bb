...
SEND_YOUR_CHANNEL = '👥Для получения подписчиков в ваш канал:\n__1️⃣. Добавьте в него этого бота\n2️⃣. Отправьте сюда юзернейм вашего канала.__'

def SEND_SUB_COUNT_1(m):
    send_sub_count  = f'🗣️Теперь отправьте нужное вам количество подписчиков.\n*🪙Доступно:* {user_balance(m.from_user.id)}'
...
def PROFILE(m):
    profile = f'👤 Пользователь: {m.from_user.first_name}\n📟 Ваш ID: `{m.from_user.id}`\n🪙 Баланс: {user_balance(m.from_user.id)} SUB COIN\n✔️ Сделано подписок: {alltime_subs(m.from_user.id)}\n➕ Получено подписчиков: {alltime_get_subs(m.from_user.id)}\n🛑 Оштрафовано всего на: {fine_count(m.from_user.id)} SUB COIN\n👥 Количество рефералов: {referals(m.from_user.id)}'
    return profile
    
    
GIVE_CHANNEL_LINK = '''🗣️*Для начала продвижения:*

1️⃣. _Добавьте этого бота в свой канал (должен быть публичным);_
2️⃣. _Пришлите сюда юзернейм этого канала. Например:_ @subcoinua'''

CANCEL_TEXT = '❌ Отменено'

BOT_NOT_IN_CHANNEL = '''🛑️Вы не добавили бота в администраторы этого канала.\n ➕ Сначала добавьте бота в нужный вам канал, а уже потом пришлите его юзернейм🤖️\n\n*После добавления бота в канал, пришлите сюда юзернейм этого канала!*'''

THIS_IS_NOT_CHANNEL = '''❌ *Это не канал!*\nПришлите сюда юзернейм канала, который вы хотите продвигать!''' 

THIS_IS_NOT_TEXT = '''🤔 *Это не юзернейм канала!*\n\n🗣 ️Пришлите сюда юзернейм канала который вы хотите продвигать.'''
...
CHANNEL_ON_PROMOTION_2 = '❌ Такой канал уже на продвижении! \n⚡ Дождитесь пока оно окончится, а потом попробуйте ещё раз.\n🤖 Добавьте другой канал или отмените действие:'
...
NO_HAVE_CHANNELS_FOR_SUBSCRIBE = f"❌ Пока нет каналов для подписки.\n\n💬А пока можете заглянуть в наш чатик: {LINK_TO_CHAT_OF_BOT}"
